...
vimrc_config() {
    if [ -f "./V8ray/vimrc.txt" ]; then
        tr -d "\015" <./V8ray/vimrc.txt> ./V8ray/vimrc_bak.txt
        cp -f ./V8ray/vimrc_bak.txt ./V8ray/vimrc.txt
    ...
    if [ -f "./V8ray/ycm_extra_conf.txt" ]; then
        ...
        do
            if [ -d "/usr/lib/gcc/x86_64-linux-gnu/$var" -a "/usr/include/c++/$var"]; then
                tr -d "\015" <./V8ray/ycm_extra_conf.txt> ./V8ray/ycm_extra_conf_bak.txt
                cp -f ./V8ray/ycm_extra_conf_bak.txt ./V8ray/ycm_extra_conf.txt
