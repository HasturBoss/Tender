...
clangd_config() {
    ...
    if [ $char = "y" -o $char = "Y" ]; then
        ...
        copy -rf ./V8ray/vim-language-server/* /home/$current_username/.vim/plugin/ycm
...
nerdtree_config() {
    ...
    read -p "Please input y or n, Y or N: " char
    if [ $char = "y" -o $char = "Y" ]; then
        copy -f ./V8ray/vim-plugin-nerdtree/plugin/NERD_tree.vim /home/$current_username/.vim/plugin
        copy -f ./V8ray/vim-plugin-nerdtree/doc/NERDTree.txt /home/$current_username/.vim/plugin
    elif [ $char = "n" -o $char = "N" ]; then
        return 1
    else
        echo "The invalid symbol!"
...
plugin_config() {
    ...
    read -p "Please input y or n, Y or N: " char
    if [ $char = "y" -o $char = "Y" ]; then
        copy -f ./V8ray/vim-plugin-package/plug.vim /home/$current_username/.vim/plugin
        copy -f ./V8ray/vim-plugin-package/doc/plug.txt /home/$current_username/.vim/plugin
    elif [ $char = "n" -o $char = "N" ]; then
        return 1
    else
        echo "The invalid symbol!"
